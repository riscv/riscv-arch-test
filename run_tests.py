...
def run_test(command: str, log_dir: Path, elf_dir: Path, elf_path: Path, verbose: bool) -> tuple[bool, Path, str]:
    ...
    with log_file.open("w") as f:
        ...
        try:
            ...
            timeout_msg = f'RVCP-SUMMARY: TEST FAILED - Test File "{elf_path}" (timeout after {5 * 60}s)'
            print(f"\n{timeout_msg}", file=f)
            print(
                f"  {red('FAIL')}  {bold(elf_path.name)} — timed out after 5 minutes"
                f"\n         Log: {dim(str(log_file))}"
            )
            return True, elf_path, timeout_msg
