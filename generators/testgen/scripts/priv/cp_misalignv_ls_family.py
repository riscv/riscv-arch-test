"""Generators for MisalignV LS misaligned coverpoints.

Implements:

- ``cp_misalignv_ls_element_misaligned`` — element LS with base address
  whose low bits are non-zero.
- ``cp_misalignv_ls_wholereg_misaligned`` — whole-register LS
...
upstreamed) SsstrictV helper module so the MisalignV PR does not depend
on SsstrictV.
"""
...
@register("cp_misalignv_ls_element_misaligned")
def make_element_misaligned(instruction: str) -> None:
    ...
    for off in range(1, 8):
        _ls_test(instruction, "cp_misalignv_ls_element_misaligned",
                  sew=sew, lmul_flag="m1", addr_offset=off)


# ---------------- ls_wholereg_misaligned ----------------

@register("cp_misalignv_ls_wholereg_misaligned")
def make_wholereg_misaligned(instruction: str) -> None:
    ...
    sew = eew if eew else 8
    for off in range(1, 8):
        _ls_test(instruction, "cp_misalignv_ls_wholereg_misaligned",
                  sew=sew, lmul_flag="m1", addr_offset=off)
