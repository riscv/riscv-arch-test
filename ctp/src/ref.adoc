...
* RVMODEL DUT-Specific Macros including:
...
*** need to identify all coverpoints whose behavior is nondeterministic, and list all acceptable test outcomes as part of the test spec.

*** sometimes these might be controlled by a parameter that allows them to become deterministic.  Even though tail-agnostic has two outcomes allowed in the spec, a DUT might only produce one of the outcomes, and the test could be simplified to check for only that outcome.
