...
[ -n "$ELF" ]      || { echo "run-veer-eh2.sh: no ELF given" >&2; exit 2; }
[ -f "$ELF" ]      || { echo "run-veer-eh2.sh: no such ELF: $ELF" >&2; exit 2; }
SIM="$SNAPSHOT/obj_dir/Vtb_top"
[ -x "$SIM" ]      || { echo "run-veer-eh2.sh: no simulator at $SIM (build it with tools/Makefile verilator-build)" >&2; exit 2; }
...
# VeeR EH2's testbench hard-codes the reset vector to 0 (tb_top.sv: "reset_vector = 32'h0;"),
...
# VeeR EH2's testbench ends a failing test with $finish, not $fatal, so the process exit
