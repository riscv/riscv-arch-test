...
`rvtest_entry_point` is the first symbol in the `.text.init` section (which is the first section), so its address is set by `TEST_BASE` at the top of the linker script. For most DUTs, `TEST_BASE` is the same as `RAM_ORIGIN` and should be set to the reset vector of your processor. Advanced users can set the starting address to a different address, run a custom bootloader, and then jump to `rvtest_entry_point` to start the test.
