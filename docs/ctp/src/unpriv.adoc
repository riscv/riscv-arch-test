...
|PC||||auipc (implicit), jal (implicit)|||||
...
The number of hazard bins for a typical R-type integer instruction is ((2 sources) * (2 hazard types) + (1 destination) * (4 hazard types)) * (8 categories) * (2 delays) = 128 bins.
