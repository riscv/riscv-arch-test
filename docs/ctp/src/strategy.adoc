...
*WPRI* fields are not tested because the test suite may be unaware of subsequently ratified extensions that use them. *WLRL* fields are tested with all legal values but no illegal values. Similarly, *WARL* fields are tested with all legal values but no illegal values. Reserved values are never tested in CSRs because the behavior is implementation-defined.
