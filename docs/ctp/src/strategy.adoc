...
*WPRI* fields should be read-only zero if not furnished by the implementation, but are not tested because the test suite may be unaware of subsequently ratified extensions that furnish them. *WLRL* fields are tested with all legal values but no illegal values. Similarly, *WARL* fields are tested with legal values. The legalization of illegal values is implementation-specific and can be difficult to convey to a reference model for multibit fields, so *WARL* fields are not tested with illegal values unless it is easy to determine their legality (e.g. 1-bit fields).
