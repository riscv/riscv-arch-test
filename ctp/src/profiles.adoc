...
** RV{32/64}IMAFDC_Zifencei_Zicclsm_Zicntr_Zihpm with no Sm machine mode, no PMP.  Zicsr instructions only access counters and floating-point CSRs. Zicclsm implies support for misaligned loads and stores.
...
* The RV2I 20 profile states "Misaligned loads and stores may be supported."  In terms of the language used in later profiles, this is interpreted as "Zicclsm is optional"
...
|ZicsrU|User CSRs|<<t-ZicsrU-coverpoints>>|||x|x|x
...
|ZicsrVF|Vector FP CSRs|<<t-ZicsrVF-coverpoints>>|||o|o|x
