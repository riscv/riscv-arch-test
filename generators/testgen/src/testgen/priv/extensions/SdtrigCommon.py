...
ETRIGGER_EXCODE_NAMES = {
    ...
    1: "iacc",  # instruction access fault
    2: "ill",  # illegal instruction
    3: "bp",  # breakpoint
    4: "lam",  # load address misaligned
    5: "lacc",  # load access fault
    6: "sam",  # store/AMO address misaligned
    7: "sacc",  # store access fault
    8: "ecu",  # ecall from U-mode
    9: "ecs",  # ecall from S-mode
    11: "ecm",  # ecall from M-mode
    12: "ipf",  # instruction page fault
    13: "lpf",  # load page fault
    15: "spf",  # store/AMO page fault
}
...
UDB_DEFINES = [
    ...
    *[f"#define UDB_EXCEPTION_{name.upper()}_SUPPORTED" for name in ETRIGGER_EXCODE_NAMES.values()],
    ...
    "#define SDTRIG_TRIGGER_BP_HANDLING",  # replaces SDTRIG_IMPRECISE_XEPC, handles etrigger and itrigger
]
...
def _mprv_as_umode_clear(scratch_reg: int) -> list[str]:
...
def _etrigger_codes_to_test(mode: str, cross_priv: bool = False) -> tuple[int, ...]:
    ...
    valid_modes = ("S", "U") if cross_priv else ("Sm", "S", "U")
    if mode not in valid_modes:
        raise ValueError(f"Unsupported {'target ' if cross_priv else ''}mode: {mode}")
    ...
    *,
    ...
    if code == 0:  # instruction address misaligned
        ...
        return [
            ...
            f"addi x{addr_reg}, x{addr_reg}, 5",
    ...
    if code == 13:  # load page fault
        ...
        if mode == "Sm":
            assert mprv_scratch_reg is not None
            lines.extend(_mprv_as_umode_clear(mprv_scratch_reg))
        return lines
        ...
        for code in codes_to_test:
            ...
            for priv_en in (0, 1):
                ...
                for raised_code in raised_codes:
                    ...
                    lines.extend(_read_trigger_hit(cfg_reg, data_reg, trig_num, mode, test_data))
            ...
            for priv_en in (0, 1):
                ...
                for medeleg_en in (0, 1):
                    for raised_code in raised_codes:
                        ...
                        lines.extend(_read_trigger_hit(cfg_reg, data_reg, trig_num, "Sm", test_data))
            lines.append(f"#endif // UDB_EXCEPTION_{name.upper()}_SUPPORTED")
        lines.extend(_disable_trigger(temp_reg, trig_num, "Sm"))
        lines.append(f"#endif // UDB_SDTRIG_ETRIGGER_SUPPORTED{trig_num}")

    # lines.append("endif") // ETRIGGER_SUPPORTED
    test_data.int_regs.return_registers([cfg_reg, data_reg, temp_reg])
    return [test_data.end_test_chunk()]
