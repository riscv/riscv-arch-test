...
DELETE this chapter and point to ACT4 documentation when ready.

TODO: This section needs a complete rewrite.  riscof no longer used.  Support tests that have expected values built in without needing a reference model.
...
Furthermore, if the self-checking tests rely on values from a reference model (as opposed to values provided by the test writer):
...
4) Implement the  RVMODEL macros for your DUT

*** See framework/examples/wally_rv64gc/rvmodel_macros.h for a trick box
