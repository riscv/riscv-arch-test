...
the certification tests. This includes putting the processor into a known
initial state (for some state, it is sufficient that it holds _any_ legal value).
...
be, the SEE must provide a means to request that they are and a means to
...
interrupts (MEI, MTI, MSI) are out of scope for RVA23.
