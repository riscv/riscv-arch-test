...
[NOTE]If the instruction is reserved only in certain architectural states, it is not required to raise an illegal instruction and Ssstrict does not test it.  For example, vector register groups that are not a multiple of LMUL are reserved, but LMUL is architectural state not encoded in the instruction, so instructions with groups not aligned to LMUL are reserved but untested even by Ssstrict.
...
*WPRI* fields are not tested. *WLRL* fields are tested with all legal values but no illegal values. Similarly, *WARL* fields are tested with all legal values but no illegal values. Reserved values are never tested in CSRs because the behavior is unpredictable.

Custom instructions and CSRs are never tested because the behavior is implementation-defined.
