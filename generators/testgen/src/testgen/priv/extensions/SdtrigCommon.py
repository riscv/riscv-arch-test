...
def _disable_all_triggers(reg: int, mode: str) -> list[str]:
    """Emit assembly disabling every trigger"""
    lines: list[str] = ["# disable all triggers for a clean state"]
    for t in range(UDB_NUM_TRIGGERS):
        lines.extend(
            [
                _load_reg(reg, t),
                _csr_access(f"csrw tselect, x{reg}", mode),
                _csr_access("csrw tdata1, x0 # disable", mode),
            ]
        )
    return lines
...
def _config_mcontrol6(
    ...
    *,
    ...
    if reset_others:
        lines.extend(_disable_all_triggers(reg, mode))
    lines.extend(
        [
