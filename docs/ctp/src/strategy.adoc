...
*WPRI* fields are not tested. *WLRL* fields are tested with all legal values but no illegal values. Similarly, *WARL* fields are tested with all legal values but no illegal values. Reserved values are never tested in CSRs because the behavior is implementation-defined.
