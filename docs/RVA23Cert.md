...
The RVA23 profile does not specify machine-mode requirements. The ACTs use a Test Supervisor Binary Interface (T-SBI) to abstract access to the Test Supervisor Execution Environment (T-SEE), in a similar manner to the RISC-V SBI. The T-SEE provides capabilities to boot the system and handle traps to machine mode. The ACTs come with a default T-SEE implementation with a trap handler that works for the machine mode described in the ratified ISA manual. If your system has additional boot requirements or implements a different machine mode, consult the [Certification Requirements Document](https://github.com/riscv/riscv-arch-test/blob/act4/docs/crd/src/rva23_crd.adoc) for information about what your T-SEE must provide.
