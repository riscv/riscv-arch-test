...
#     "// PMP read access on the instruction region is guaranteed because the core",
#     "// already fetched the instruction successfully before raising the illegal-",
#     "// instruction trap.",
...
_FAST_HANDLER_PREFIX: list[str] = [
    ...
    "\taddi x2, x2, SIG_STRIDE",
    "    # Branchless mepc advance — reads faulting instruction from *mepc.",
    "    # PMP access is safe: the fetch already succeeded.",
    "    # advance = ((bits[1:0] >> 1) + 1) << 1  →  4 if uncompressed, 2 if compressed.",
    "\tcsrr t0, mepc",
    "\tlw   t0, 0(t0)                  # load faulting instruction word from *mepc",
    "\tandi t0, t0, 3                  # t0 = bits[1:0]",
    "\tsrli t0, t0, 1                  # t0 = 1 (uncompressed 0b11) or 0 (compressed)",
    ...
    "othertrap:",
    "\tcsrr t1, mtval",
    "\tbgez t0, Mtrampoline            # msb clear = exception, hand off to full handler",
    "",
    "ssstrict_test_body:",
