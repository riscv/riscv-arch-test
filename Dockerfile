...
#
# Includes:
#   - riscv-gnu-toolchain  (GCC 15 / Binutils 2.44, built from source)
#   - uv                   (Python project / venv manager)
#   - sail-riscv           (RISC-V reference model)
#   - podman               (required by ACT4 for riscv-unified-db)
#
# Usage:
#   docker build -t riscv-act4 .          # slow: compiles GCC from source
...
#
# All the heavy compile-time deps never reach the final image
...
# Runtime deps only:
#   - libgmp/libmpc/libmpfr/libz: needed by GCC binaries at runtime
#   - libexpat / libncurses: needed by gdb / objdump
