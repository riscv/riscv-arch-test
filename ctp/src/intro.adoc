...
* Extensions: named and ratified RISC-V extensions, such as RV32I, M, Zbb, Zicsr, or Svadu.  Many extensions define instructions or CSRs, but some only define parameters (e.g. Svadu requires that the *** parameter )
...
* RVMODEL DUT-Specific Macros: A set of macros that perform _semihosting_/_trick box_ functions such as booting the DUT, sending interrupts, and logging results. The macros is defined in <<RVMODEL DUT-Specific Macros>>.
