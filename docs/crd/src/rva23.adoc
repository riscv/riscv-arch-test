...
1) Check if the trap is an T-SBI call and if so, perform the requested T-SBI function and return from the `ecall`.
...
3) Check if the trap is a test of undelegated traps to M-mode, and if so, write/compare a trap signature of the standard values expected in `mepc`, `mcause`, `mtval` and `mstatus.MPV/MPP/MPIE/MPELP`  *** check for full list
...
Question about whether to also test every trap scenario both delegated and undelegated when the test expects delegation.
...
| `mip` | Interrupts are controlled with RVMODEL macros.  SIEP, STIP, and SSIP may be controlled with T-SBI accesses to mip.
