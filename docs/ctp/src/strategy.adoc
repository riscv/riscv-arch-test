...
[NOTE]
====
When an extension implies sub-extensions (e.g. `C` implies `Zca`/`Zcf`/`Zcd`; `A` implies `Zaamo`/`Zalrsc`), the `implemented_extensions` list must include *both the superset and all applicable sub-extensions*.  UDB validates this, and the test framework selects test suites by sub-extension name.
====
