...
csri_config = InstructionTypeConfig(
    required_params={"rd", "temp_reg", "temp_val", "immval"}, imm_bits=5, imm_signed=False
)


def zicsr_accessi(instr_name: str, rd: int, temp_reg: int, immval: int) -> str:
    ...
    if instr_name in ["csrrw", "csrrwi"] or rd == 0:
        read_only_access = f"li x{rd}, 0x42 # avoid write to read-only CSR, or inconsistent result with rd = 0"
    else:
        read_only_access = (
            f"{instr_name} x{temp_reg}, RVTEST_TEST_CSR, 0\n"
            f"{instr_name} x{rd}, RVTEST_TEST_CSR, 0\n"
            f"sub x{rd}, x{rd}, x{temp_reg}  # check that the read-only CSR value has incremented\n"
        )
...
def format_csri_type(
    ...
    test = [
        "// perform operation",
        zicsr_accessi(instr_name, params.rd, params.temp_reg, params.immval),
    ]
    check = [
