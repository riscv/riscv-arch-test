...
The DUT also needs to know how to boot the core, terminate a test with a success code, send a message to a console, and cause interrupts.  These DUT-specific behaviors are defined by a <<rvmodel-macros, RVMODEL DUT-Specific Macros>>, which are an assembly-language library implementing an API using DUT-specific hardware.
...
* Optional DUT configuration files https://github.com/riscv-non-isa/riscv-arch-test/blob/cvw/configs/duts/cvw/cvw-rv64gc[(CORE-V Wally Example)] providing parameter values to configurable RTL
...
[NOTE]
Ssstrict states that the illegal instruction is contained in the supervisor-mode trap handler.  Certification tests do not exercise the execution environment, so the illegal instruction trap to machine mode.
...
TODO: Generate this table from norm rules via YAML nwhen Ssstrict is tagged.
...
TODO: this section is no longer accurate. Determine whether a summary of the test approach belongs in the CTP and move to a new section.
