...
VECTOR_WIDEN_PREFIXES = ("Vx", "Vls", "Vf", "Zvfhmin", "Zvfbfmin", "Zvfbfwma", "Zvzip")
...
def _sew_variants_for(arch: str) -> list[str] | None:
    ...
    the base entry.
    """
    if any(prefix in arch for prefix in ("Vx", "Vls", "Zvbb", "Zvkb", "Zvzip")):
        return ["8", "16", "32", "64"]
    if "Vf" in arch:
...
def _filter_testplans(
    ...
    such as Vx, Zvkb, or Zvzip to match their per-SEW variants.
    """
