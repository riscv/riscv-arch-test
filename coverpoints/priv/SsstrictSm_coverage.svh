...
covergroup SsstrictSm_misa_cg with function sample(ins_t ins);
    option.per_instance = 0;
    `include "general/RISCV_coverage_standard_coverpoints.svh"
    ...
    misa_disable_instr: coverpoint ins.current.insn {
        ...
        // C extension: c.addi (op=01, funct3=000, nzimm!=0)
        ...
        // I/E extension: add with upper register (x16-x31 as rs1, op=0110011)
        wildcard bins add_upper = {32'b0000000_?????_1????_000_?????_0110011};
    }

    // Cross: which misa bit was off crossed with which instruction was used
