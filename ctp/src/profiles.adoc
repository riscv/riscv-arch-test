...
** RV{32/64}IMAFDC_Zifencei_Zicntr_Zihpm with no Sm machine mode, no PMP.  Zicsr instructions only access counters and floating-point CSRs. Optional support for misaligned loads/stores (MISALIGNED_LDST parameter).
* MC100 Microcontroller Profile (https://github.com/riscv-software-src/riscv-unified-db/releases/tag/v0.1.0[MC100-32-CRD.pdf]): Phase 1
...
* RVA23 Profile (https://riscv-software-src.github.io/riscv-unified-db/pdfs/AC200-CRD.pdf[AC200 CRD]): Phase 2
...
* All certificates for profiles that support interrupts (Phase 1 and above) require that the platform be able to set and clear via software all interrupt pending bits defined by the certificate.
...
|VF|Vector FP CSRs|<<t-VF-coverpoints>>|||o|o|x
...
|SvHCBO|Hypervisor Virtual Memory + CBO|<<t-SvHCBO-coverpoints>>|||o|o|x
...
|Zama16b|Main memory PMA supports misaligned atomics within 16-byte regions|||||x
...
* Non-conforming machine mode; without this, it is very difficult to write the tests
* Multicore or multi-master systems; atomics and memory consistency are only lightly tested on a single core
...
* Testing extensions that have been disabled with `misa` fields; low ROI
...
*** move Zama16b to tested when misaligned atomics are added (ExceptionsZaamo)
