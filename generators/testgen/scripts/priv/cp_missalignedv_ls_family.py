"""Generators for MissalignedV LS misaligned coverpoints.

Implements:

- ``cp_missalignedv_ls_element_misaligned`` — element LS with base address
  whose low bits are non-zero.
- ``cp_missalignedv_ls_wholereg_misaligned`` — whole-register LS
...
upstreamed) SsstrictV helper module so the MissalignedV PR does not depend
on SsstrictV.
"""
