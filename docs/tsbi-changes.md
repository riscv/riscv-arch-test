...
- Review test plans. Determine if any coverpoints need to go into a SSm suite because they run in machine mode (e.g. those that set medeleg=0)
  - Update coverpoints for SSm
  - Move tests for these coverpoints to a SSm suite
...
- Replace accesses to higher privilege with add_tsbi calls producing TSBI_CSR_WRITE etc.
