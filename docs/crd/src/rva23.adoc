...
NOTE: The profiles already explain what is required and what is optional. Retstaing the requirements here can lead to confusion.

NOTE: While it is permitted for an implementation to also support RV32I instructions, this is neither a mandated requirement or explicitly specified option. Therefore, there are no RV32I tests included in RVA23 certification. We encourage those who have implemented RV32I instructions in their processor to test these with the appropriate RISC-V ACTs.
...
NOTE: The abstraction is not just for custom M-mode ISAs. It allows for hiding differences in implementations as well allowing lower privileges to make requests to M-mode. Furthermore, it is common for implementations to comply with the RISC-V M-Mode ISA while adding implementation-defined features.
...
NOTE: There is no requirement that any traps can be delegated. But if they can be, the SEE must provide a means to request that they are and a means to to determine which requested traps have been delegated.
...
* `ecall` from S/M-mode must not be delegated because the T-SBI must trap to M-mode to handle these calls.
...
* Traps causes that hardware cannot delegate (e.g., bits of `m*deleg` that are read-only zero.)
