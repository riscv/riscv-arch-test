...
- `make tests` already regenerates when anything under `generators/testgen/src/` or `testplans/` changes (the stamp depends on them). To force it, `rm work/stamps/testgen.stamp`. **Never run `make clean-tests` with `EXTENSIONS` restricted** (including via a local Makefile default): it deletes the checked-in `tests/rv32i tests/rv32e tests/rv64i tests/rv64e coverpoints/unpriv` outputs and a filtered `make tests` does not regenerate them. Recover with `git checkout -- tests/rv32i tests/rv32e tests/rv64i tests/rv64e coverpoints/unpriv`. Use `make clean-tests tests` only with `EXTENSIONS=` (everything).
...
- Privileged CSR instructions from a lower mode go through `tsbi_call("csrw mstatus, x{reg}")` from `testgen.asm.tsbi`, which marshals rs1/rs2/rd through a1/a2/a0 and re-encodes the instruction. a0-a2 are reserved out of the priv register pool for this. The handler executes only instructions listed in `tsbi_instr_table` (`tests/env/rvtest_trap_handler.h`); a CSR missing from that table fails at run time with `T-SBI ERROR: requested instruction not found in tsbi_instr_table`. `medeleg` is deliberately not in the table.
