...
- Suites that enable address translation (the Sv family) run the test driver from the boot identity map (`rvtest_identity_map`), which has no U bit, so U-mode cannot fetch the driver's code. Register a U-accessible virtual alias of the code region once with `LA(a0, Mtramptbl_sv)` followed by `SAVE_AREA_SETUP(va_rvtest_code_begin, rvtest_code_begin, code, LEVEL)` (`sv/generate.py` `begin_sv_test`). While an alias is registered and `satp.MODE` is not Bare, `RVTEST_TSBI_GOTO_UMODE` resumes in the alias and `RVTEST_TSBI_GOTO_SMODE`/`_MMODE` from the alias resume in the identity-mapped code (`TSBI_RELOCATE_EPC` in `rvtest_trap_handler.h`), so tests use the same macros as every other suite. S-mode accesses stay on the identity map.
- Page faults are delegated to S-mode (and ecall from U-mode reaches the S-mode T-SBI dispatch), so coverpoints for converted S/U suites sample `scause`/`stval`; faults taken in M-mode (e.g. `mstatus.MPRV` accesses in an Sm suite) still use `mcause`/`mtval`.
