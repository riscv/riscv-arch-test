...
The hazard receives the source or sends the destination value to a _Secondary Instruction_ (SI).  All relevant
...
The categories of SI are given in <<t-secondary-instruction-categories>>.

[[t-secondary-instruction-categories]]
.Secondary Instruction Categories
[options=header]
|===
|Category|ALU|Memory|Control|CSR|Mult|Float|VectorInt|VectorFloat
|Extension||||Zicsr|M|F|Zve32x, Zvl64b|Zve32f, Zvl64b
|Int Dest (rd)|auipc, li|lw, amoswap.w (Zaamo)|jal|csrr|mul, div|fcvt.w.s, fmv.x.w, fclass.s, feq.s|vmv.x.s|
|Int Source|add (rs1/2), addi (rs2)|sw (rs2), amoswap.w (rs2, Zaamo)|csrw (rs1)|beq (rs1/2)|mul (rs1/2), div (rs1/2)|fcvt.s.w (rs1), fmv.w.x (rs1)
|vadd.vx (rs1), vmv.v.x (rs1)|
|FP Dest (fd)||flw||||fmadd.s, fdiv.s, fli.s (Zfa), fsqrt.d (D), fsqrt.h (Zfh), fsqrt.q (Q)||vmv.f.s
|FP Source||fsw (fs2)||||fmadd.s (fs1/2/3), fdiv.s (fs1/2), fli.s (fs1), fsqrt.d (fs1, D), fsqrt.h (fs1, Zfh), fsqrt.q (fs1, Q)||vfadd.vf (fs1), vmv.v.f (fs1)
|Vector Dest (vd)||vle32.v|||||vadd.vi|vfadd.vf
|Vector Source||vse32.v(vs2)|||||vadd.vv (vs1/2), vadd.vi (vs2), vmacc.vv (vs1/2/vd)|vfadd.vv (vs1/2), vfsqrt.v (vs2), vfmacc.vv (vs1/2/vd)
|Memory Dest||sw, amoswap.w (Zaamo), cbo.zero (Zicboz)||||fsw|vse32.v|
|Memory Source||lw, amoswap.w (Zaamo)||||fsw|vle32.v|
|CSR Source|n/a|||||csrr fflags|csrr vcsr|
|CSR Dest|n/a|||||csrw fflags|csrw vcsr|
|===

When multiple representative instructions are listed, one is randomly selected from the list for each test. Certain categories and cdertain instructions in a category (amoswap, fli, fsqrt.d/h/q) only may be selected if the relevant extension is present.  Different source instructions consume one, two, or three sources.  Loads, stores, and atomics always use rs1 as a pointer to a scratch memory location holding a random value. Subword load, store, and atomic IUTs use a random naturally aligned offset in the range [0, 3]. Tests assume mstatus.FS/VS are dirty, if applicable.  Loads and atomics have a memory source operand.  Stores, atomics, and cbo.zero have a memory destination operand.

The test cases are as follows:

* If the IUT has {int/fp/vector/memory/CSR} source operand:
** For each IUT source
*** If IUT has a destination, hazard_types = {RAW, WAW}.  Else hazard_types = RAW.
*** For each hazard_type
**** For each supported SI category that can write the source type (gated by #ifdef EXT_SUPPORTED)
...
****** Pick random representative SI from category, and random unused source registers for SI.
****** Initialize IUT and SI sources to random values
****** If hazard_type = RAW, SI dest = random IUT source
****** If hazard_type = WAW, SI dest = IUT dest
****** Perform SI
****** If delay > 0, perform delay dependent instructions
****** Perform IUT
****** SIGUPD IUT dest or otherwise test IUT result

* If the IUT has {int/fp/vector/memory/CSR} destination operand:
** For each hazard_type in {RAW, WAR}
*** For each supported SI category that can read the source type (gated by #ifdef EXT_SUPPORTED)
**** For each delay in (0, random[1,3])
***** Pick random unused registers for IUT source(s) and destination from X1-x15, f0-f31, v0-v31.  Ok to duplicate source registers.
***** Pick random representative SI from category, and random unused source registers for SI.
***** Initialize IUT and SI sources to random values
***** If hazard_type = RAW, random SI source = IUT source
***** If hazard_type = WAR, SI dest = random IUT source
***** Perform IUT
***** If delay > 0, perform delay dependent instructions
***** Perform SI
***** SIGUPD SI dest or otherwise test SI result
***** For WAR, also SIGUPD IUT dest to confirm IUT got proper source

Hazard coverpoints are named cp_hazard_{hazard_type}_{IUT source/dest}_{SI category}_{delay}. IUT source/dest is one of {rs1, rs2, rd, fs1, fs2, fs3, fd, vs1, vs2, vd, memr, memw, csrr, csrw}.
