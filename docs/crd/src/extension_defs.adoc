...
This appendix contains information about RISC-V extensions. Just because an extension is listed in this appendix doesn't necessarily mean that it's required for a particular certificate. Please refer to the "Extensions" section of the relevant CRD for information about which extensions are required for that certificate.
