...
covergroup PMPZca_cg with function sample(ins_t ins, logic [7:0] pmpcfg [63:0], logic [14:0] pmp_hit, logic [XLEN-1:0] pmpaddr [62:0]);
  ...
  `ifdef G_IS_0
    ...
    pmpaddr_consecutive_na4: coverpoint ((ins.current.csr[12'h3B2] == ((`PMP_REGION_START + 8) >> 2)) &&
                                         (ins.current.csr[12'h3B1] == ((`PMP_REGION_START + 4) >> 2)) &&
                                         (ins.current.csr[12'h3B0] == (`PMP_REGION_START >> 2))) {
      bins first_three_regions = {1};
    }
