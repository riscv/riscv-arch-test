...
def _generate_supervisor_sti_tests(test_data: TestData) -> list[str]:
    ...
    deleg=1 (cp_supervisor_sti_deleg, priv_mode_s):
      Enter S-mode via mret. For STCE=1: stimecmp=0 is written in M-mode
      before RVTEST_GOTO_LOWER_MODE, then the testcase label and a nop execute
      in S-mode; the interrupt may fire after entry to S-mode. For STCE=0:
      stimecmp=0 written in M-mode (no STI since STCE=0 disables Sstc),
      sample at nop in S-mode. MIE in S-mode is controlled via MPIE; SIE via
      csrsi/csrci in S-mode.
                    ...
                    for mideleg_val in mideleg_values:
                        for stie in [0, 1]:
                            ...
                            if stce:
                                ...
                                lines += set_stimecmp_zero()  # stimecmp=0; clear both halves on RV32 too
                            ...
                            # use the existing SV coverpoint name for testcase attribution
                            test_name = "cp_supervisor_sti_deleg"
