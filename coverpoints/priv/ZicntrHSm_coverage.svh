...
//
// Counter reads and htimedelta observed from M-mode with the hypervisor extension.
...
`define COVER_ZICNTRHSM
covergroup ZicntrHSm_cg with function sample(ins_t ins);
    option.per_instance = 0;
    `include "general/RISCV_coverage_standard_coverpoints.svh"

    // Each counter read or written with its enable bit alone set or alone clear
    ...
    csrr_time: coverpoint ins.current.insn[31:20] iff (ins.current.insn ==? CSRR) {
        ...
        `ifdef UDB_MXLEN_32
            bins read_timeh = {CSR_TIMEH};
        `endif
    }
    `ifdef UDB_MXLEN_64
        htimedelta: coverpoint ins.current.csr[CSR_HTIMEDELTA] {
    `else
    ...
    // mcounteren, hcounteren and scounteren hold the same walking value
    cp_mhscounteren_access_m: cross csraccess, counters_mcounteren, priv_mode_m
        iff (ins.current.csr[CSR_MCOUNTEREN] == ins.current.csr[CSR_HCOUNTEREN] &&
             ins.current.csr[CSR_MCOUNTEREN] == ins.current.csr[CSR_SCOUNTEREN]);
    cp_delta_m: cross csrr_time, htimedelta, priv_mode_m;
endgroup

function void zicntrhsm_sample(int hart, int issue, ins_t ins);
    ZicntrHSm_cg.sample(ins);
endfunction
