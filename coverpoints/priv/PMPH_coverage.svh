...
//
// PMP enforcement with the hypervisor extension: PMP checked from VS-mode and VU-mode, by the
// hypervisor loads and stores, and on the addresses that two-stage translation produces.
...
covergroup PMPH_cg with function sample(ins_t ins, logic [16*`UDB_MXLEN-1:0] pack_pmpaddr, logic [29:0] pmpcfg_a, [7:0] pmpcfg [63:0], logic [14:0] pmp_hit);
...
//-------------------------------------------------------
  ...
  // mstatus.MPP and MPV select VS-mode or VU-mode for MPRV accesses
  ...
  `ifdef UDB_MXLEN_64
    mpv_mstatus: coverpoint ins.prev.csr[CSR_MSTATUS][39] {
  `else
    mpv_mstatus: coverpoint ins.prev.csr[CSR_MSTATUSH][7] {
  `endif
    bins set = {1};
  }
...
//-------------------------------------------------------
  ...
  cp_cfg_X: cross priv_mode_vs_vu, legal_lxwr, exec_instr, addr_in_region ;
  cp_cfg_R: cross priv_mode_vs_vu, legal_lxwr, read_instr, addr_in_region ;
  cp_cfg_W: cross priv_mode_vs_vu, legal_lxwr, write_instr, addr_in_region ;

  cp_cfg_A_off_jalr: cross priv_mode_vs_vu, cfg_A_off, exec_instr, addr_in_region ;
  cp_cfg_A_off_lw: cross priv_mode_vs_vu, cfg_A_off, read_instr_lw, addr_in_region ;
  cp_cfg_A_off_sw: cross priv_mode_vs_vu, cfg_A_off, write_instr_sw, addr_in_region ;

  // Access at start of region, start - 4, start + 4, highest word in region, just beyond top of the region
  cp_cfg_A_napot_jalr: cross priv_mode_vs_vu, cfg_A_napot, exec_instr, addr_offset_napot ;
  cp_cfg_A_napot_lw: cross priv_mode_vs_vu, cfg_A_napot, read_instr_lw, addr_offset_napot ;
  cp_cfg_A_napot_sw: cross priv_mode_vs_vu, cfg_A_napot, write_instr_sw, addr_offset_napot ;

  `ifdef UDB_PMP_NA4_SUPPORTED
    // Access at start of address, that address - 4, just beyond top of the region.
    cp_cfg_A_na4_jalr: cross priv_mode_vs_vu, cfg_A_na4, exec_instr, addr_offset_na4 ;
    cp_cfg_A_na4_lw: cross priv_mode_vs_vu, cfg_A_na4, read_instr_lw, addr_offset_na4 ;
    cp_cfg_A_na4_sw: cross priv_mode_vs_vu, cfg_A_na4, write_instr_sw, addr_offset_na4 ;
  `endif

  // Access at address, address-4, address-g, address-g-4.
  cp_cfg_A_tor_jalr: cross priv_mode_vs_vu, cfg_A_tor, exec_instr, addr_offset_tor ;
  cp_cfg_A_tor_lw: cross priv_mode_vs_vu, cfg_A_tor, read_instr_lw, addr_offset_tor ;
  cp_cfg_A_tor_sw: cross priv_mode_vs_vu, cfg_A_tor, write_instr_sw, addr_offset_tor ;

  cp_mprv_jalr: cross priv_mode_m, mprv_mstatus, mpp_mstatus, mpv_mstatus, lxwr, exec_instr, standard_region, addr_in_napot_region ;
  cp_mprv_lw: cross priv_mode_m, mprv_mstatus, mpp_mstatus, mpv_mstatus, lxwr, read_instr_lw, standard_region, addr_in_napot_region ;
  cp_mprv_sw: cross priv_mode_m, mprv_mstatus, mpp_mstatus, mpv_mstatus, lxwr, write_instr_sw, standard_region, addr_in_napot_region ;

  cp_pmpaddr_access_vs: cross priv_mode_vs, csrrw, pmpaddr_entries ;
  cp_pmpcfg_access_vs: cross priv_mode_vs, csrrw, pmpcfg_entries ;
  cp_pmpaddr_access_vu: cross priv_mode_vu, csrrw, pmpaddr_entries ;
  cp_pmpcfg_access_vu: cross priv_mode_vu, csrrw, pmpcfg_entries ;
...
`ifdef UDB_MXLEN_64
  `define PMPH_GVA_SUPERPAGE(addr) addr[63:30]
  `define PMPH_VS_SUPERPAGE 5
  `define PMPH_G_SUPERPAGE 11
`else
  `define PMPH_GVA_SUPERPAGE(addr) addr[31:22]
  `define PMPH_VS_SUPERPAGE 'h241
  `define PMPH_G_SUPERPAGE 'h341
`endif

// hsv, hlv and hlvx from M-mode and HS-mode, against each legal XWR (vsatp and hgatp Bare) and
// through two-stage translation with PMP denying a page table or the final address
covergroup PMPH_hlv_cg with function sample(ins_t ins, logic [7:0] pmpcfg [63:0], logic [14:0] pmp_hit, int pmp_entry);
  option.per_instance = 0;
  `include "general/RISCV_coverage_standard_coverpoints.svh"
  ...
  hlv_w_hsv_w_hlvx_wu: coverpoint ins.current.insn {
    wildcard bins hlv_w   = {HLV_W};
    wildcard bins hsv_w   = {HSV_W};
    wildcard bins hlvx_wu = {HLVX_WU};
  }

  // Each unlocked legal XWR in the entry that lxwr_walk_body gives it
  unlocked_lxwr: coverpoint {pmpcfg[0],pmpcfg[1],pmpcfg[2],pmpcfg[3],pmpcfg[4],pmpcfg[5],pmp_hit[5:0]} {
    wildcard bins cfg_0000 = {54'b????????????????????????????????????????00011000_100000};
    wildcard bins cfg_0001 = {54'b????????????????????????????????00011001????????_?10000};
    wildcard bins cfg_0011 = {54'b????????????????????????00011011????????????????_??1000};
    wildcard bins cfg_0100 = {54'b????????????????00011100????????????????????????_???100};
  ...
  addr_in_napot_region: coverpoint (ins.current.rs1_val & `PMP_ADDR_LOWMASK) {
    bins at_region = {`PMP_NAPOT_REGION_START & `PMP_ADDR_LOWMASK};
  }
  ...
  mapping: coverpoint `PMPH_GVA_SUPERPAGE(ins.current.rs1_val) {
    bins vs = {`PMPH_VS_SUPERPAGE};
    bins g  = {`PMPH_G_SUPERPAGE};
  }
  // PMP entry denying a page table (0-5) or the final address (6), or none
  pmp_scenario: coverpoint pmp_entry {
    bins none  = {-1};
    bins vlvl0 = {0};
    bins hlvl0 = {1};
    `ifdef UDB_MXLEN_64
...
covergroup PMPH_guest_cg with function sample(ins_t ins, int pmp_entry);
  ...
  // PMP denies the supervisor physical address, or the guest virtual addresses of both mappings
  pmp_final: coverpoint pmp_entry {
    bins spa = {6};
    bins gva = {7};
  }
  pmp_spa: coverpoint pmp_entry {
    bins spa = {6};
  }
  // PMP denies the page table that holds an invalid leaf
  pmp_leaf_table: coverpoint pmp_entry {
    bins vlvl0 = {9};
    bins hlvl0 = {10};
  }
  // PMP denies a page table below the root
  pmp_table: coverpoint pmp_entry {
    bins vlvl0 = {0};
    bins hlvl0 = {1};
    `ifdef UDB_MXLEN_64
      bins vlvl1 = {2};
  ...
  access_fault: coverpoint {ins.current.csr_wb[CSR_MCAUSE], ins.current.csr[CSR_MCAUSE][4:0]} {
    bins access_fault = {6'b100001, 6'b100101, 6'b100111};
  }
...
function void pmph_sample(int hart, int issue, ins_t ins);
  ...
  `endif
  // The lowest-numbered enabled PMP entry below the background entry names the PMPH.py scenario
  pmp_entry = -1;
  for (int i = 10; i >= 0; i--) begin
    if (pmpcfg[i][4:3] != 2'b00) pmp_entry = i;
  end

  PMPH_cg.sample(ins, pack_pmpaddr, pmpcfg_a, pmpcfg, pmp_hit);
  PMPH_hlv_cg.sample(ins, pmpcfg, pmp_hit, pmp_entry);
  PMPH_guest_cg.sample(ins, pmp_entry);
endfunction
