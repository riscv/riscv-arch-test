...
#          cmd = self.compile_cmd.format(testentry['isa'].lower().replace('zicsr', ' ', 2), self.xlen, test, elf, compile_macros)
      ...
      # echo statement.
          if self.target_run:
            ...
            simcmd = self.dut_exe + ' --misaligned -m8796093022208 --isa={0} --pmpgranularity={1} +signature={2} +signature-granularity=4 {3}'.format(self.isa, self.granularity, sig_file, elf)
