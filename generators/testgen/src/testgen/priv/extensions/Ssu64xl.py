...
def _generate_ssu64xl_tests(test_data: TestData) -> list[str]:
    ...
    save_reg, val_reg, check_reg, orig_reg = test_data.int_regs.get_registers(4)
    ...
    lines.extend(
        [
            ...
            f"and x{save_reg}, x{save_reg}, x{val_reg}",
            f"LI(x{val_reg}, {2 << 32})",  # UXL=2 → bit 33 set
            f"or x{save_reg}, x{save_reg}, x{val_reg}",
    ...
    test_data.int_regs.return_registers([save_reg, val_reg, check_reg, orig_reg])
    return lines
