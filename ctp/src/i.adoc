...
Note that there are presently no normative rules for uncompressed hint instructions.  All of the hints involve instructions writing rd = `x0`.  Every instruction that could be a hint has a cp_rd coverpoint and thus inherently tests that the hint runs and does not change `x0`.
