...
def _generate_csr_illegal_accesses(test_data: TestData) -> list[str]:
    ...
    lines = [
        comment_banner(
            coverpoint,
            "All CSR ops to mstateenN CSRs from U-mode — must trap (M-only CSRs)",
        )
    ]
    ...
    # Base 4 CSRs — unconditional so symbols are defined exactly once.
    ...
    # High-half CSRs only on RV32 — single guard block, no if/else.
...
def _generate_walking_ones(test_data: TestData) -> list[str]:
    ...
    lines = [
        comment_banner(
            coverpoint,
            "Walking-1 and walking-0 patterns on every mstateenN CSR via CSRRW from M-mode",
        )
    ]
...
def _generate_envcfg(test_data: TestData) -> list[str]:
    ...
    lines = [
        comment_banner(
            coverpoint,
            "CSR ops on senvcfg from S-mode with mstateen0.envcfg (bit 62) enabled and disabled",
        )
    ]
    ...
    # Test enabled (state=1) first, then disabled (state=0) so the tool sees
    # senvcfg accessed under both envcfg_state values.
    for state in [1, 0]:
        bit_action = "CSRS" if state == 1 else "CSRC"
        lines.extend([
            "",
            f"\t# Set mstateen0.envcfg = {state} from M-mode, then drop to S-mode",
            "#if __riscv_xlen == 64",
        ...
        for op in CSR_OPS:
            lines.extend([
                "",
                test_data.add_testcase(f"senvcfg_{op.lower()}_envcfg{state}", coverpoint, covergroup),
...
def _generate_aia(test_data: TestData) -> list[str]:
    ...
    for state in [0, 1]:
        ...
        # RV64 uses sie/sip; RV32 uses sieh/siph
...
def _generate_jvt_access(test_data: TestData) -> list[str]:
    ...
    temp_reg, ones_reg = test_data.int_regs.get_registers(2, exclude_regs=[0])
    JVT_BIT_MASK = 1 << 2  # bit 2 of mstateen0
...
def _generate_jvt_lower_mode(test_data: TestData) -> list[str]:
    ...
    modes = [("umode", _enter_umode), ("smode", _enter_smode)]
            ...
            for op in CSR_OPS:
                lines.extend([
                    ...
                    test_data.add_testcase(
                        f"jvt_{op.lower()}_jvt{state}_{mode_label}", coverpoint, covergroup
                    ),
                    _csr_insn(op, temp_reg, "jvt", ones_reg),
                    "\tnop",
                ])
...
def _generate_context(test_data: TestData) -> list[str]:
    ...
    lines = [
        comment_banner(
            coverpoint,
            "CSR ops on scontext with mstateen0.context (bit 57) disabled and enabled",
        )
    ]
    ...
    for state in [0, 1]:
        ...
        for op in CSR_OPS:
            lines.extend([
                "",
                test_data.add_testcase(f"scontext_{op.lower()}_context{state}", coverpoint, covergroup),
...
def _generate_p1p13(test_data: TestData) -> list[str]:
    ...
    lines = [
        comment_banner(
            coverpoint,
            "CSR ops on hedelegh with mstateen0.p1p13 (bit 56) disabled and enabled",
        )
    ]
    ...
    for state in [0, 1]:
        ...
        for op in CSR_OPS:
            lines.extend([
                "",
                test_data.add_testcase(f"hedelegh_{op.lower()}_p1p13_{state}", coverpoint, covergroup),
...
def _generate_srmcfg(test_data: TestData) -> list[str]:
    ...
    lines = [
        comment_banner(
            coverpoint,
            "CSR ops on srmcfg with mstateen0.srmcfg (bit 55) disabled and enabled",
        )
    ]
    ...
    for state in [0, 1]:
        ...
        for op in CSR_OPS:
            lines.extend([
                "",
                test_data.add_testcase(f"srmcfg_{op.lower()}_srmcfg{state}", coverpoint, covergroup),
...
def _generate_ctr(test_data: TestData) -> list[str]:
    ...
    lines = [
        comment_banner(
            coverpoint,
            "CSR ops on sctrdepth/sctrstatus with mstateen0.ctr (bit 54) disabled and enabled",
        )
    ]
    ...
    for state in [0, 1]:
        ...
        for csr in ctr_csrs:
            for op in CSR_OPS:
                lines.extend([
                    "",
                    test_data.add_testcase(f"{csr}_{op.lower()}_ctr{state}", coverpoint, covergroup),
                    _csr_insn(op, temp_reg, csr, ones_reg),
                    "\tnop",
                ])
...
def _generate_fcsr(test_data: TestData) -> list[str]:
    """cp_fcsr — M-mode CSR ops on fcsr under misa.F and mstateen0.fcsr states."""
    coverpoint = "cp_fcsr"
    covergroup = "Smstateen_cg"
    ...
    # ignore_bins in the SV covergroup exclude fcsr_zero+F_set AND fcsr_zero+F_clear
    # so only fcsr_one + F_set is a valid bin; test that state.
    lines.extend([
        "",
    ...
    # Also test fcsr_zero to hit the ignore_bins side (coverage will mark them ignored)
    lines.extend([
        "",
        "\t# mstateen0.fcsr = 0",
        f"\tLI(x{temp_reg}, {FCSR_BIT_MASK})",
...
def _generate_fcsr_lower(test_data: TestData) -> list[str]:
    """cp_fcsr_lower — S/U-mode CSR ops on frm/fflags/fcsr under mstateen0.fcsr states."""
...
def _generate_fcsr_lower_fp_instrs(test_data: TestData) -> list[str]:
    """cp_fcsr_lower_fp_instrs — FP instructions from U-mode under mstateen0.fcsr states."""
    ...
    lines = [
        comment_banner(
            coverpoint,
            "FP instructions from U-mode with mstateen0.fcsr (bit 1) disabled and enabled",
        )
    ]
    ...
    # Each entry: (asm_template, label_prefix)
    # {temp} and {scratch} are substituted with register numbers.
    fp_instrs = [
    ...
    lines.extend([f"\tLA(x{scratch_reg}, scratch)  # scratch memory for flw"])

    for state in [0, 1]:
        bit_action = "CSRC" if state == 0 else "CSRS"
        lines.extend([
            "",
            f"\t# mstateen0.fcsr = {state}, umode",
            f"\tLI(x{temp_reg}, {FCSR_BIT_MASK})",
            f"\t{bit_action}(mstateen0, x{temp_reg})",
        ])
        lines.extend(_enter_umode(test_data, temp_reg))
        for insn_template, label in fp_instrs:
            insn = (insn_template
                    .replace("{temp}", str(temp_reg))
                    .replace("{scratch}", str(scratch_reg)))
            lines.extend([
                "",
                test_data.add_testcase(f"{label}_fcsr{state}_umode", coverpoint, covergroup),
                f"\t{insn}",
                "\tnop",
            ])
        lines.extend(_return_mmode(test_data, temp_reg))
...
def make_smstateen(test_data: TestData) -> list[str]:
    ...
    lines.append("#ifdef SM1P13_AND_H_SUPPORTED")
    lines.extend(_generate_p1p13(test_data))
    lines.append("#endif  // SM1P13_AND_H_SUPPORTED")
    ...
    # cp_fcsr, cp_fcsr_lower, cp_fcsr_lower_fp_instrs — only when Zfinx present
